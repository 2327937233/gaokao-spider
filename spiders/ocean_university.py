...
实际使用前，请：
1. 打开中国海洋大学本科招生网，确认录取分数栏目实际URL
2. 使用浏览器开发者工具（F12）查看文章列表的HTML结构
3. 根据实际结构修改下面的 CSS 选择器

中国海洋大学本科招生网参考地址：
- 首页: https://bkzs.ouc.edu.cn/
- 历年分数线栏目可能需要手动查找确认
"""
...
class OceanUniversitySpider(BaseSpider):
    """中国海洋大学爬虫"""
    ...
    def discover_score_pages(self):
        ...
        if not entry_url:
            logger.warning("中国海洋大学 score_entry_url 未配置，请在 config.py 中填写")
            return []

        logger.info(f"中国海洋大学 - 访问录取分数入口: {entry_url}")
        ...
        logger.info(f"中国海洋大学 - 共发现 {len(unique)} 个相关页面")
        return unique
    ...
    def parse_page(self, html, url):
        ...
        logger.info(f"中国海洋大学 - 从 {url} 解析到 {len(all_data)} 条记录")
        return all_data
